...
const EditAnnonceAssistant = ({ route, navigation }) => {
  const [annonce, setAnnonce] = useState(route.params.annonce);

  const handleUpdate = async () => {
    try {
      const response = await fetch(`http://10.0.2.2:8080/api/assistants/${annonce.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        navigation.navigate('VoirAnnonceAssistant', { refresh: true });
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        onChangeText={(text) => setAnnonce({ ...annonce, location: text })}
...
export default EditAnnonceAssistant;
