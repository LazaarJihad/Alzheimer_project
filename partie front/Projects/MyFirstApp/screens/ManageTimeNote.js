...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Image, Alert, TouchableWithoutFeedback } from 'react-native';
...
const ManageTimeNote = () => {
  ...
  const { assistantId, username, email, city, phoneNumber, userType ,id} = route.params;
  ...
  const fetchNotes = async () => {
    try {
      ...
      console.log('Notes:', data);  // Ajout du log
      setNotes(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Erreur lors de la récupération des notes :', error);
      Alert.alert('Erreur', 'Erreur lors de la récupération des notes.');
    }
  };
 ...
 const fetchTasks = async () => {
     try {
       ...
       const data = await response.json();
       console.log('Tasks:', data);  // Ajout du log
       setTasks(Array.isArray(data) ? data : []);
     } catch (error) {
       console.error('Erreur lors de la récupération des tâches :', error);
       Alert.alert('Erreur', 'Erreur lors de la récupération des tâches.');
     }
   };
  ...
  const handleViewAll = () => {
    if (activeButton === 'tasks') {
      navigation.navigate('AfficherTask', { assistantId, username, email, city, phoneNumber, userType ,id });
  ...
  const handleTaskClick = (taskId) => {
    if (assistantId) {
      navigation.navigate('AddTask', { assistantId, username, email, city, phoneNumber, userType,id });
    } else {
      Alert.alert('Erreur', "Aucun assistant trouvé pour ajouter des tâches.");
    }
  };
    ...
    return (
      <View style={[styles.contentContainer, { marginTop: 50 }]}>
        <View style={styles.contentHeader}>
          <TouchableOpacity onPress={handleTaskClick}>
            <Text style={styles.contentTitle}> Ajouter Tâches </Text>
          </TouchableOpacity>
  ...
  const renderNotes = () => {
    return (
      <View style={[styles.contentContainer, { marginTop: 50 }]}>
        ...
        <ScrollView>
          {notes.length === 0 ? (
            <Text style={styles.noTaskText}>Aucune note disponible.</Text>
          ) : (
            notes.map(note => (
      ...
      <View style={styles.container}>
        ...
        <View style={styles.mainContentContainer}>
          <View style={styles.buttonsContainer}>
            <TouchableOpacity
              style={[styles.button, activeButton === 'tasks' && styles.activeButton]}
              onPress={() => handleButtonPress('tasks')}
            >
              <Text style={styles.buttonText}>Mes tâches</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.button, activeButton === 'notes' && styles.activeButton]}
